...
| [Action Recognition with I3D](action_recognition) | Action recognition to identify video/webcam footage from what actions are performed (e.g. "running", "opening a bottle") and at what respective start/end times. Please note, that we also have a R(2+1)D implementation of action recognition that you can find under [scenarios](../scenarios).| |
| [Tracking deployment with Azure ML](tracking_deployment) | Multi-object tracking deployment using Azure Machine Learning pipelines with parallelized model inference. This pipeline uses the FairMOT implementation used for tracking under [scenarios](../scenarios).| |
